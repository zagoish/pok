...
function NobleChoiceContent({ pendingNobles, claimNoble }: { pendingNobles: Noble[]; claimNoble: (nobleId: NobleId) => void }) {
  return (
    <>
      ...
      <div className="noble-choice__options">
        {pendingNobles.map((noble) => (
          <button type="button" key={noble.id} onClick={() => claimNoble(noble.id)}>
...
export default function NoblePanel({ state, claimNoble }: NoblePanelProps) {
  ...
  const pendingNobles = state.pendingNobleIds
    .map((nobleId) => NOBLES.find((noble) => noble.id === nobleId))
    .filter((noble) => noble !== undefined)
  const choiceContent = <NobleChoiceContent pendingNobles={pendingNobles} claimNoble={claimNoble} />
    ...
    <section className="noble-panel" aria-labelledby="noble-heading">
      ...
      {state.pendingNobleIds.length > 0 ? (
        choiceOpen ? (
          ...
          <div className="noble-choice noble-choice--inline" aria-labelledby="noble-choice-heading">
            {choiceContent}
          </div>
        )
      ) : null}
